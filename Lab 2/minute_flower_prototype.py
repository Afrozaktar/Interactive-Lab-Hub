...
def wind_sway(minute_value):
    """
    Gentle repeating wind motion.

    The stem smoothly moves:

        straight -> left -> straight -> right -> straight

    over a 4-minute cycle.

    Because minute_value includes seconds, the movement
    remains smooth instead of jumping between positions.
    """

    cycle = minute_value % 4.0

    sway = math.sin(
        cycle * 2 * math.pi / 4
    )

    # Maximum bend of the stem.
    max_angle = math.radians(6)

    return sway * max_angle
...
def draw_leaves(
    ...
    Leaves follow the movement of the stem.
    """
...
def draw_minutes_flower(
    ...
    flower_cx = flower_pos[0]
    flower_cy = flower_pos[1]
    ...
    if bloom > 0.15:
        ...
        draw.ellipse(
            (
                throat_pos[0] - throat_w,
                throat_pos[1] - 1 * SCALE,
                throat_pos[0] + throat_w,
                throat_pos[1] + throat_h
            ),
            fill=LIP_HIGHLIGHT
        )
    ...
    if bloom < 0.30:
        ...
        # Bud follows the flower position and sway.
        bud_pos = transform(
            0,
            -10 * SCALE,
            sway_angle,
